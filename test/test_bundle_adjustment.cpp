...
OptSummary run_ba(const char *filename, double time_in_sec, int num_iterations) {
    ...
    const char *filename2 = "../data/data2.csv";
    write_data(filename2, ba_data, bearing_noise, kf_noise, lm_noise);
    ...
    unsigned int new_kf = 0;  // The solver will exit if this is 1
    auto ba_prob = construct_ba(&ba_data, ceres_options, &new_kf);
